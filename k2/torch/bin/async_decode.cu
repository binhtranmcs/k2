/**
 * Copyright      2021  Xiaomi Corporation (authors: Fangjun Kuang, WeiKang)
 *
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    std::vector<std::unique_ptr<k2::DecodeStateInfo>> current_states_info(FLAGS_num_streams);
    ...
    for (int32_t i = 0; i < num_waves; ++i) {
      ...
      positions[i] = end;
      auto sub_output = nnet_output.index({i, Slice(start, end), Slice()});

      namespace F = torch::nn::functional;
      sub_output = F::pad(sub_output, F::PadFuncOptions({0, 0, 0, chunk_size - end + start}).mode(torch::kConstant));

      current_nnet_output.push_back(sub_output);

      if (num_frame.size() >= FLAGS_num_streams) break;
    }
    if (num_frame.size() == 0) break; // finished
    while (num_frame.size() < FLAGS_num_streams) {
      num_frame.push_back(0);
      auto opts = torch::TensorOptions().dtype(nnet_output.dtype()).device(nnet_output.device());
      current_nnet_output.push_back(torch::zeros({chunk_size, nnet_output.size(2)}, opts));
    }
    ...
    sup.insert("num_frames",
               torch::from_blob(num_frame.data(), {FLAGS_num_streams}, torch::kLong)
                   .to(torch::kInt));
    ...
    decoder.Decode(dense_fsa, &fsa, &graph_arc_map, &current_states_info);

    for (int32_t i = 0; i < current_wave_ids.size(); ++i) {
