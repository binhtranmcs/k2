...
namespace k2 {
    namespace jni {
        ...
        void init(JNIEnv *env, jobject, jstring jModelPath, jstring jBpePath) {
            ...
            const char *pModelPath = (env)->GetStringUTFChars(jModelPath,
                                                              nullptr);
            std::string modelPath = std::string(pModelPath);
            LOGI("model path: %s\n", modelPath.c_str());
//            K2_LOG(INFO) << "model path: " << modelPath;
            ...
            int max_contexts = 10;
            int max_states = 50;
            rnnt_decoding::RnntDecodingConfig config(vocab_size, context_size,
                                                     beam, max_states,
                                                     max_contexts);
            g_config = std::make_shared<rnnt_decoding::RnntDecodingConfig>(
                    config);
        }
        ...
        jstring decode(JNIEnv *env, jobject, jfloatArray jWaveform) {
            ...
            auto wave_data = torch::from_blob(waveform.data(), {size},
           ...
           // K2_LOG(INFO) << "Compute features";
            ...
            auto input_lengths =
                    torch::from_blob(num_frames.data(), {1}, torch::kLong)
                            .to(torch::kInt)
                            .to(device);
            ...
            auto outputs =
                    g_module->run_method("encoder_forward", features,
                                      input_lengths).toTuple();
            ...
            std::vector<std::shared_ptr<rnnt_decoding::RnntDecodingStream>> current_streams;
            current_streams.push_back(
                    rnnt_decoding::CreateStream(g_decoding_graph->fsa));

            LOGI("Build rnnt streams.\n");
            auto streams =
                    rnnt_decoding::RnntDecodingStreams(current_streams,
                                                       *g_config);

            LOGI("Decoding.\n");
            LOGI("streams %d\tdim %d\tsize0 %d\n", streams.NumStreams(), encoder_outs.dim(), encoder_outs.size(0));
            ...
            int32_t frames = min(num_frames[0] / subsampling_factor, encoder_outs.size(1));
