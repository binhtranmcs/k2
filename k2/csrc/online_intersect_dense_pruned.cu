...
OnlineIntersectDensePruned::OnlineIntersectDensePruned(
...
void OnlineIntersectDensePruned::Intersect(std::shared_ptr<DenseFsaVec> &b_fsas,
                                           bool is_final /*= false*/) {
  /*
  ...
  */
  ...
  int32_t prune_num_frames = 30, prune_shift = 20;
...
std::unique_ptr<FrameInfo> OnlineIntersectDensePruned::InitialFrameInfo() {
  ...
  if (a_fsas_.Dim0() == 1) {
    ...
    ans->states = Ragged<StateInfo>(
        start_states.shape, Array1<StateInfo>(c_, start_states.NumElements()));
...
Array1<float> OnlineIntersectDensePruned::GetPruningCutoffs(
    Ragged<float> &arc_end_scores) {
  NVTX_RANGE(K2_FUNC);
...
Ragged<ArcInfo> OnlineIntersectDensePruned::GetArcs(int32_t t,
                                                    FrameInfo *cur_frame) {
  NVTX_RANGE(K2_FUNC);
...
std::unique_ptr<FrameInfo> OnlineIntersectDensePruned::PropagateForward(
    int32_t t, FrameInfo *cur_frame) {
...
*/
void OnlineIntersectDensePruned::SetBackwardProbsFinal(FrameInfo *cur_frame) {
...
/*
  ...
  cur_frame->states which we need to keep, and 0s for others.
     @param [out] cur_frame_arcs_keep   An array, created by the caller,
                      ...
                      and 0s for others.
*/
void OnlineIntersectDensePruned::PropagateBackward(
    FrameInfo *cur_frame, FrameInfo *next_frame,
    Array1<char> *cur_frame_states_keep, Array1<char> *cur_frame_arcs_keep) {
  ...
  Ragged<float> arc_backward_prob(
      cur_frame->arcs.shape, Array1<float>(c_, cur_frame->arcs.NumElements()));
  ...
  const int32_t *cur_states_row_ids1 = cur_frame->states.shape.RowIds(1).Data();
 ...
 */
void OnlineIntersectDensePruned::PruneTimeRange(int32_t begin_t,
                                                int32_t end_t) {
  NVTX_RANGE(K2_FUNC);
    ...
    for (int32_t t = end_t - 1; t >= begin_t; --t) {
      ...
      Array1<char> this_states_keep = renumber_states.Keep().Arange(
                   ...
                   this_arcs_keep = renumber_arcs.Keep().Arange(
                       old_arcs_offsets_data[i], old_arcs_offsets_data[i + 1]);
      FrameInfo *cur_frame = frames_[t].get();
      PropagateBackward(cur_frame, frames_[t + 1].get(), &this_states_keep,
                        &this_arcs_keep);
  ...
  for (int32_t i = 0; i < num_t; i++) {  // i corresponds to "t_offset".
    ...
    RaggedShape arcs_shape =
        RaggedShape3(&row_splits1, &row_ids1, -1, &row_splits2, &row_ids2, -1);
    int32_t t = begin_t + i;
